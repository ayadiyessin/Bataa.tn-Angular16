...
@Component({
  selector: 'app-autre-informatique',
  templateUrl: './autre-informatique.component.html',
  styleUrls: ['./autre-informatique.component.css']
})
export class AutreInformatiqueComponent {
  ...
  GetProduitByScat() {
    // this.idcourant = "1";
    this.PS.getProduitsNonValiderBySCategorie(this.idcourant[2]).subscribe((result) => {
      this.dataa = result;
  ...
  GetrechercherProduits() {
    ...
    if(this.etatProduit=="tous"){
      ...
      this.PS.rechercheProduit(this.idcourant[2], this.etatProduit)
      .subscribe(
        (data) => {
          if(data.length==0){
            Swal.fire({
